...
const Playlist = () => {
    ...
    let playlist;
    useEffect(() => {
        let ind = user?.result?.playlists.findIndex((p) => p?.key === id);
        if(ind !==-1){
            playlist = user?.result?.playlists[ind];
    ...
    <Box sx={{overflowY: 'scroll',height: '82vh'}}>
        <Box sx={{background: `linear-gradient(0deg, ${color1} 0%, ${color3} 100%)`,display: 'flex',flexDirection: 'row',alignItems: 'flex-end',padding: '25px 30px',flex: '0 0 250px'}}>
            <Box height={{sm: '196px',md: '236px'}} width={{sm: '196px',md: '236px'}} sx={{display: 'flex',flexWrap: 'wrap',boxShadow: `0px 0px 20px ${color4}`}}>
                {!songs?.length ? <img 
                    style={{flex: '1 1',minHeight:'196px',maxHeight: '236px',minWidth:'196px',maxWidth: '236px'}}
                    src={defaultImage}
                    alt='cover'
                />:<>
                <img 
            ...
            <Box sx={{color: 'white',marginLeft: '20px'}}>
                ...
                <Typography sx={{fontWeight: 700,fontSize: '90px'}}>{playlist?.name}</Typography>
                <Box sx={{display: 'flex',flexDirection: 'row'}}>
                    {user && <Link to='/account'><Typography className='link' sx={{color:'white',fontWeight: 'bold',marginRight: '5px'}}>{user?.result?.username}</Typography></Link>}
                    <Typography>{' • '+(songs?.length || 0)+' '}songs</Typography>
                </Box>
            </Box>
        </Box>
